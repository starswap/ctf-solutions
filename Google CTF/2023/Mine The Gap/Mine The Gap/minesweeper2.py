...
CELL_SIZE = 18
print(pygame.transform.scale(pygame.image.load("img/flag.png"), (CELL_SIZE, CELL_SIZE)))
...
def too_many(r, c):
    if validate_grid[r][c].state >= 1 and validate_grid[r][c].state <= 8:
        ...
        for i in range(len(dr)): # go through all neighbours
            newr = r + dr[i]
            newc = c + dc[i]
            if validate_grid[newr][newc] in (10, 11):
                count += 1
...
def solve(r, c):
    ...
    if validate_grid[r][c].state == 9:
        ...
        if not(ok): # try not flagging
            # print("here2")
            validate_grid[r][c].click()
            ok = True
            for i in range(len(dr)): # all neighbours
                if too_few(r + dr[i], c + dc[i]):
                    ok = False
                    break
