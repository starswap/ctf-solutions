...
def socket_manager(to_socket, from_socket):
    with connect(SOCK_URL) as websocket:
        message = websocket.recv()
        from_socket.put(message)
        while True:
            to_send = to_socket.get()
            if to_send == "DONE":
                break
            websocket.send(to_send)
            response = websocket.recv()
            from_socket.put(response)

def computer(to_socket, from_socket):
    global game_state, R, C
    received_messages = []

    message = from_socket.get()
    ...
    while not(game_state["numKeysRetrieved"] == 40):
        ...
        mark_mines()
        mark_safe()
        go_to_safe(to_socket, from_socket)
        go_to_keys(to_socket, from_socket)
...
if __name__ == "__main__":
    ...
    to_socket = multiprocessing.Queue()
    from_socket = multiprocessing.Queue()

    # Create sender and receiver processes
    socket_mgr = multiprocessing.Process(target=socket_manager, args=(to_socket,from_socket))
    comptr = multiprocessing.Process(target=computer, args=(to_socket,from_socket))
...
def go_to_type(typeOf, to_socket, from_socket):
    global game_state
        ...
        for c, sq in enumerate(row):
            if sq == typeOf:
                ...
                if route is not None:
                    do_steps(route, to_socket, from_socket)

def go_to_safe(to_socket, from_socket):
    go_to_type("safe", to_socket, from_socket)

def go_to_keys(to_socket, from_socket):
    go_to_type("key", to_socket, from_socket)
...
def do_steps(route, to_socket, from_socket):
    global game_state
    to_socket.put('\n'.join(route))
    new_state = json.loads(from_socket.get())
